...
describe('BrandAwareness Component', () => {
	const factory = (routeName = RouteName.CAMPAIGN_CREATE, storeState: { section?: FormSection; currentCampaignType?: CampaignType } = {}) => {
		...
		if (routeName === RouteName.CAMPAIGN_EDIT) {
			formCampaignStore.campaign = brandAwarenessCampaign
		}
	...
	it('initializes with campaign data in edit mode', async () => {
		...
		const vm = wrapper.vm as any
		expect(vm.model.title).toBe(brandAwarenessCampaign.title.default)
		expect(vm.model.description).toBe(brandAwarenessCampaign.description)
		expect(vm.model.category).toBe(brandAwarenessCampaign.category.id)
		expect(vm.model.holding).toBe(brandAwarenessCampaign.holding.id)
		expect(vm.model.advertiser).toBe(brandAwarenessCampaign.advertiser.id)
	})
	...
	it('calls updateCampaign on submit in edit mode', async () => {
		...
		expect(formCampaignStore.updateCampaign).toHaveBeenCalledWith({
			...vm.model,
			slug: brandAwarenessCampaign.slug,
		})
	})
