...
import type { ICreateSpecialProjectCampaignData,ISpecialProjectCampaign } from '@/modules/Partner/views/FormCampaign/api'
...
const defaultModel: ICreateSpecialProjectCampaignData = {
	title: '',
	description: '',
	category: undefined,
	affiliateNetwork: -1,
...
let model = reactive<ICreateSpecialProjectCampaignData>({
	...defaultModel,
})
...
const rules = computed<FormRules<ICreateSpecialProjectCampaignData>>(() => ({
	title: [required],
...
const campaignToModel = (campaignData: ISpecialProjectCampaign) => {
	...
	try {
		...
		if (route.name === RouteName.CAMPAIGN_EDIT) {
			if (campaign.value && campaign.value.visible !== modelVisibleStatus.value) {
				await campaignsStore.changeStatus(campaign.value.slug, CampaignType.SPECIAL_PROJECT, AdEntityType.CAMPAIGNS)
			}
...
const showCloseDialog = computed(() => {
	if (route.name === RouteName.CAMPAIGN_EDIT) {
		return !isEqual(model, campaignToModel(formCampaignStore.campaign as ISpecialProjectCampaign))
	}
	if (route.name === RouteName.CAMPAIGN_CREATE) {
...
const setFormData = (campaignData: ISpecialProjectCampaign | null) => {
...
onMounted(() => {
	setFormData(formCampaignStore.campaign as ISpecialProjectCampaign)
})

watch(campaign, (campaignData) => {
	setFormData(campaignData as ISpecialProjectCampaign)
})
