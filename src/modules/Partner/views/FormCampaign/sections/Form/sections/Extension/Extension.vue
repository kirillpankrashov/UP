...
import type { ICreateExtensionCampaignData,IExtensionCampaign } from '@/modules/Partner/views/FormCampaign/api'
...
const defaultModel: ICreateExtensionCampaignData = {
	title: '',
	description: '',
	category: undefined,
	holding: undefined,
...
let model = reactive<ICreateExtensionCampaignData>({
	...defaultModel,
})
...
const rules = computed<FormRules<ICreateExtensionCampaignData>>(() => ({
	title: [required],
...
const campaignToModel = (campaignData: IExtensionCampaign) => {
	...
	try {
		...
		if (route.name === RouteName.CAMPAIGN_EDIT) {
			if (campaign.value && campaign.value.visible !== modelVisibleStatus.value) {
				await campaignsStore.changeStatus(campaign.value.slug, CampaignType.EXTENSION, AdEntityType.CAMPAIGNS)
			}
...
const showCloseDialog = computed(() => {
	if (route.name === RouteName.CAMPAIGN_EDIT) {
		return !isEqual(model, campaignToModel(formCampaignStore.campaign as IExtensionCampaign))
	}
	if (route.name === RouteName.CAMPAIGN_CREATE) {
...
const closeAndReturnRoute = computed(() => RouteName.EXTENSION_CAMPAIGNS)

const setFormData = (campaignData: IExtensionCampaign | null) => {
...
onMounted(() => {
	setFormData(formCampaignStore.campaign as IExtensionCampaign)
})

watch(campaign, (campaignData) => {
	setFormData(campaignData as IExtensionCampaign)
})
