<template>
  ...
  >
    ...
    <DashboardSection
      ...
      data-test="special-project-name-section"
    >
      <FormTitle
        v-model="model"
    ...
    <DashboardSection
      ...
      data-test="special-project-files-section"
    >
      <FormCreative
        v-model="model"
        :format="formAdsetStore.adset?.format.id ?? AdFormat.SP_FULLSCREEN"
        :max-size-mb="maxFileSize"
    ...
    <DashboardSection
      data-test="special-project-data-section"
    ...
    <DashboardSection
      data-test="special-project-analytics-section"
...
import {  SP_CUSTOM_MAX_SIZE_MB, SP_PIP_MAX_SIZE_MB, SP_VIDEO_MAX_SIZE_MB } from '@/core/consts'
...
import type { ICreateSpecialProjectCreativeData, ISpecialProjectCreative } from '@/modules/Partner/views/FormCreative/api'
import {
	FormAdTag,
	FormChatbotText,
	FormImpression,
...
import { FormCreative } from '@/modules/Partner/views/FormCreative/components/form/type/SpecialProject'
...
const creative = computed(() => formCreativeStore.creative as ISpecialProjectCreative | null)
...
const rules = computed<FormRules<ICreateSpecialProjectCreativeData>>(() => ({
	title: [required],
...
const defaultModel: ICreateSpecialProjectCreativeData = {
...
let model = reactive<ICreateSpecialProjectCreativeData>({
	...defaultModel,
})

const creativeToModel = (creativeData: ISpecialProjectCreative): ICreateSpecialProjectCreativeData => {
...
const maxFileSize = computed(() => {
	switch (formAdsetStore.adset?.format.id) {
		case AdFormat.SP_CUSTOM:
			return SP_CUSTOM_MAX_SIZE_MB
		default:
			return SP_VIDEO_MAX_SIZE_MB
	}
})
...
const creativeRequirements = computed(() => {
	...
	return dictStore.campaigns?.formatRequirements.find(requirement => requirement.format === formAdsetStore.adset?.format.id)?.requirements.join('<br/>') ?? ''
})
	...
	try {
		...
		if (route.name === RouteName.CREATIVE_EDIT) {
			if (creative.value && creative.value.visible !== modelVisibleStatus.value) {
				await campaignsStore.changeStatus(creative.value.slug, CampaignType.SPECIAL_PROJECT, AdEntityType.CREATIVES)
			}
		...
		Logger.error('Error saving Special Project creative', true, err)
	}
	finally {
		sending.value = false
	}
...
const showCloseDialog = computed(() => {
	if (route.name === RouteName.CAMPAIGN_EDIT) {
		return !isEqual(model, creativeToModel(formCreativeStore.creative as ISpecialProjectCreative))
	}
...
const closeAndReturnRoute = computed(() => RouteName.SPECIAL_PROJECT_CREATIVES)

const setFormData = (creativeData: ISpecialProjectCreative | null) => {
	if (!creativeData) {
...
onMounted(() => {
	setFormData(formCreativeStore.creative as ISpecialProjectCreative)
})

watch(creative, (creativeData) => {
	setFormData(creativeData as ISpecialProjectCreative)
})
