...
import FormTargetingCountries from '../FormTargetingCountries.vue'
...
describe('FormAdset FormTargetingCountries', () => {
	const factory = async (
		model = { targeting: { countries: { list: [], exclude: false } } },
		disabled = false,
	) => {
		const wrapper = mount(FormTargetingCountries, {
			global: {
	...
	it('disables select when disabled=true', async () => {
		...
		const select = wrapper.find('[data-test="adset-form-targeting-countries-select"]')
	...
	it('shows "only selected" label when countries selected and exclude=false', async () => {
		const wrapper = await factory({
			targeting: { countries: { list: ['US', 'RU'] as never[], exclude: false } },
		})
		expect(wrapper.text()).toContain('adset.targeting.form.countries.label')
	...
	it('shows "except selected" label when countries selected and exclude=true', async () => {
		const wrapper = await factory({
			targeting: { countries: { list: ['US'] as never[], exclude: true } },
		})
		expect(wrapper.text()).toContain('adset.targeting.form.countries.label')
	...
	it('sets tag-type to success when exclude=false', async () => {
		const wrapper = await factory({
			targeting: { countries: { list: [], exclude: false } },
		})
	...
	it('sets tag-type to danger when exclude=true', async () => {
		const wrapper = await factory({
			targeting: { countries: { list: [], exclude: true } },
		})
		const select = wrapper.findComponent({ name: 'ElSelect' })
	...
	it('emits onInput when select changes', async () => {
		const wrapper = await factory()
		await wrapper.find('[data-test="adset-form-targeting-countries-select"]').trigger('change')
