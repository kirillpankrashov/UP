...
<template>
  <SpecialProjectCreativesSkeleton
    v-if="isFetching"
    :columns="columns"
  />
...
import { ElPagination, ElTable } from '@/components/element-plus'
import type { ISpecialProjectCreative } from '@/modules/Partner/views/Campaigns/api'
...
import SpecialProjectCreativesSkeleton from './SpecialProjectCreativesSkeleton.vue'
...
const items = computed(() => creatives.value.items as ISpecialProjectCreative[])
...
const columns = ref<typeof defaultColumns>(filters?.[CampaignType.SPECIAL_PROJECT]?.[AdEntityType.CREATIVES] ? filters?.[CampaignType.SPECIAL_PROJECT]?.[AdEntityType.CREATIVES] : defaultColumns)
