  ...
  >
    ...
    >
      ...
      <ElTableColumn
        ...
        :formatter="(row: IExtensionCampaign) => row.ctr.toFixed(2) + '%'"
        width="100"
      />
      <ColumnDaysRemaining
      ...
      <ElTableColumn
        ...
        :formatter="(row: IExtensionCampaign) => row.advertiser.title"
        width="150"
      ...
      <ElTableColumn
        ...
        :formatter="(row: IExtensionCampaign) => formatNumber(row.channels, false, row.advertiser.wallet.currency.code)"
        width="100"
      />
      <ElTableColumn
        v-if="columns.clicks"
        :label="t('campaigns.tables.columns.clicks')"
        :formatter="(row: IExtensionCampaign) => formatNumber(row.clicks, false, row.advertiser.wallet.currency.code)"
        width="100"
      />
...
import type { IExtensionCampaign } from '@/modules/Partner/views/Campaigns/api'
...
import ExtensionCampaignsSkeleton from './ExtensionCampaignsSkeleton.vue'
...
const items = computed(() => campaigns.value.items as IExtensionCampaign[])
...
const columns = ref<typeof defaultColumns>(filters?.[CampaignType.EXTENSION]?.[AdEntityType.CAMPAIGNS] ? filters?.[CampaignType.EXTENSION]?.[AdEntityType.CAMPAIGNS] : defaultColumns)
