  ...
  >
    ...
    >
      ...
      <ElTableColumn
        ...
        :formatter="(row: IPerformanceCampaign) => row.ctr.toFixed(2) + '%'"
        width="100"
      />
      <ColumnDaysRemaining
      ...
      <ElTableColumn
        ...
        :formatter="(row: IPerformanceCampaign) => row.advertiser.title"
        width="150"
      ...
      <ElTableColumn
        ...
        :formatter="(row: IPerformanceCampaign) => formatNumber(row.channels, false, row.advertiser.wallet.currency.code)"
        width="100"
      />
      <ElTableColumn
        v-if="columns.clicks"
        :label="t('campaigns.tables.columns.clicks')"
        :formatter="(row: IPerformanceCampaign) => formatNumber(row.clicks, false, row.advertiser.wallet.currency.code)"
        width="100"
      />
      <ElTableColumn
        v-if="columns.externalId"
        :label="t('campaigns.tables.columns.externalId')"
        :formatter="(row: IPerformanceCampaign) => row.externalId"
        width="200"
        show-overflow-tooltip
      />
...
import type { IPerformanceCampaign } from '@/modules/Partner/views/Campaigns/api'
...
import PerformanceCampaignsSkeleton from './PerformanceCampaignsSkeleton.vue'
...
const items = computed(() => campaigns.value.items as IPerformanceCampaign[])
...
const columns = ref<typeof defaultColumns>(filters?.[CampaignType.PERFORMANCE]?.[AdEntityType.CAMPAIGNS] ? filters?.[CampaignType.PERFORMANCE]?.[AdEntityType.CAMPAIGNS] : defaultColumns)
