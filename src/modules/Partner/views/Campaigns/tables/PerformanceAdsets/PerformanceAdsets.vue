  ...
  >
    ...
    >
      ...
      <ElTableColumn
        ...
        :formatter="(row: IPerformanceAdset) => formatNumber(row.channels, false, row.campaign.advertiser.wallet.currency.code)"
        width="100"
      />
      <ColumnBudget
        v-if="columns.budget"
      ...
      <ElTableColumn
        ...
        :formatter="(row: IPerformanceAdset) => row.ctr.toFixed(2) + '%'"
        width="100"
      />
      <ColumnDaysRemaining
      ...
      <ElTableColumn
        ...
        :formatter="(row: IPerformanceAdset) => formatNumber(row.clicks, false, row.campaign.advertiser.wallet.currency.code)"
        width="100"
      />
      <ColumnFormat
        v-if="columns.format"
...
import type { IPerformanceAdset } from '@/modules/Partner/views/Campaigns/api'
...
import PerformanceAdsetsSkeleton from './PerformanceAdsetsSkeleton.vue'
...
const items = computed(() => adsets.value.items as IPerformanceAdset[])
...
const columns = ref<typeof defaultColumns>(filters?.[CampaignType.PERFORMANCE]?.[AdEntityType.ADSETS] ? filters?.[CampaignType.PERFORMANCE]?.[AdEntityType.ADSETS] : defaultColumns)
