  ...
  >
    ...
    >
      ...
      <ElTableColumn
        ...
        :formatter="(row: IBrandAwarenessAdset) => formatNumber(row.channels, false, row.campaign.advertiser.wallet.currency.code)"
        width="100"
      />
      <ColumnBudget
        v-if="columns.budget"
      ...
      <ElTableColumn
        ...
        :formatter="(row: IBrandAwarenessAdset) => row.ctr.toFixed(2) + '%'"
        width="100"
      />
      <ColumnDaysRemaining
      ...
      <ElTableColumn
        ...
        :formatter="(row: IBrandAwarenessAdset) => formatNumber(row.clicks, false, row.campaign.advertiser.wallet.currency.code)"
        width="100"
      />
      <ColumnFormat
        v-if="columns.format"
...
import type { IBrandAwarenessAdset } from '@/modules/Partner/views/Campaigns/api'
...
import BrandAwarenessAdsetsSkeleton from './BrandAwarenessAdsetsSkeleton.vue'
...
const items = computed(() => adsets.value.items as IBrandAwarenessAdset[])
...
const columns = ref<typeof defaultColumns>(filters?.[CampaignType.BRAND_AWARENESS]?.[AdEntityType.ADSETS] ? filters?.[CampaignType.BRAND_AWARENESS]?.[AdEntityType.ADSETS] : defaultColumns)
