import { vi } from 'vitest'

import { RouteName } from '@/modules/Partner/router'
import { brandAwarenessCreative } from '@/modules/Partner/views/Campaigns/api/getBrandAwarenessCreatives/fixtures/brandAwarenessCreative'
import { runPartnerCampaignsTableSuite } from '@/modules/Partner/views/Campaigns/tables/tests/runPartnerCampaignsTableSuite'

import BrandAwarenessCreatives from '../BrandAwarenessCreatives.vue'

vi.mock('@/core/helpers')
vi.mock('@/modules/Partner/views/Campaigns/api')

const mockPush = vi.fn()
const routeQueryRef: { current: Record<string, string | string[]> } = { current: {} }
const routeNameRef: { current: RouteName } = { current: RouteName.BRAND_AWARENESS_CREATIVES }

vi.mock('vue-router', async () => {
	const actual = await vi.importActual<typeof import('vue-router')>('vue-router')

	return {
		...actual,
		useRouter: vi.fn(() => ({
			push: mockPush,
			currentRoute: {
				value: {
					get query() {
						return routeQueryRef.current
					},
				},
			},
		})),
		useRoute: vi.fn(() => ({
			get name() {
				return routeNameRef.current
			},
			get query() {
				return routeQueryRef.current
			},
		})),
	}
})

runPartnerCampaignsTableSuite({
	describeTitle: 'Partner Campaigns BrandAwarenessCreatives Component',
	Component: BrandAwarenessCreatives,
	defaultRouteName: RouteName.BRAND_AWARENESS_CREATIVES,
	collectionKey: 'creatives',
	skeletonStubName: 'BrandAwarenessCreativesSkeleton',
	skeletonDataTest: 'brand-awareness-creatives-skeleton-stub',
	fixtureItem: brandAwarenessCreative,
	mockPush,
	routeQueryRef,
	routeNameRef,
})
