<template>
  <BrandAwarenessCreativesSkeleton
    v-if="isFetching"
    :columns="columns"
  />
  ...
  >
    ...
    >
      ...
      <ElTableColumn
        ...
        :formatter="(row: IBrandAwarenessCreative) => row.ctr.toFixed(2) + '%'"
        width="100"
      />
      <ColumnFormat
        v-if="columns.format"
...
import type { IBrandAwarenessCreative } from '@/modules/Partner/views/Campaigns/api'
...
import BrandAwarenessCreativesSkeleton from './BrandAwarenessCreativesSkeleton.vue'
...
const items = computed(() => creatives.value.items as IBrandAwarenessCreative[])
...
const columns = ref<typeof defaultColumns>(filters?.[CampaignType.BRAND_AWARENESS]?.[AdEntityType.CREATIVES] ? filters?.[CampaignType.BRAND_AWARENESS]?.[AdEntityType.CREATIVES] : defaultColumns)
