  ...
  >
    ...
    >
      ...
      <ElTableColumn
        ...
        :formatter="(row: IPrerollAdset) => formatNumber(row.channels, false, row.campaign.advertiser.wallet.currency.code)"
        width="100"
      />
      <ColumnBudget
        v-if="columns.budget"
      ...
      <ElTableColumn
        ...
        :formatter="(row: IPrerollAdset) => row.ctr.toFixed(2) + '%'"
        width="100"
      />
      <ColumnDaysRemaining
      ...
      <ElTableColumn
        ...
        :formatter="(row: IPrerollAdset) => formatNumber(row.clicks, false, row.campaign.advertiser.wallet.currency.code)"
        width="100"
      />
      <ColumnFormat
        v-if="columns.format"
...
import type { IPrerollAdset } from '@/modules/Partner/views/Campaigns/api'
...
import PrerollAdsetsSkeleton from './PrerollAdsetsSkeleton.vue'
...
const items = computed(() => adsets.value.items as IPrerollAdset[])
...
const columns = ref<typeof defaultColumns>(filters?.[CampaignType.PREROLL]?.[AdEntityType.ADSETS] ? filters?.[CampaignType.PREROLL]?.[AdEntityType.ADSETS] : defaultColumns)
