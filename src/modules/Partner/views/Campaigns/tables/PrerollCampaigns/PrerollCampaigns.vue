  ...
  >
    ...
    >
      ...
      <ElTableColumn
        ...
        :formatter="(row: IPrerollCampaign) => row.ctr.toFixed(2) + '%'"
        width="100"
      />
      <ColumnDaysRemaining
      ...
      <ElTableColumn
        ...
        :formatter="(row: IPrerollCampaign) => row.advertiser.title"
        width="150"
      ...
      <ElTableColumn
        ...
        :formatter="(row: IPrerollCampaign) => formatNumber(row.channels, false, row.advertiser.wallet.currency.code)"
        width="100"
      />
      <ElTableColumn
        v-if="columns.clicks"
        :label="t('campaigns.tables.columns.clicks')"
        :formatter="(row: IPrerollCampaign) => formatNumber(row.clicks, false, row.advertiser.wallet.currency.code)"
        width="100"
      />
...
import type { IPrerollCampaign } from '@/modules/Partner/views/Campaigns/api'
...
import PrerollCampaignsSkeleton from './PrerollCampaignsSkeleton.vue'
...
const items = computed(() => campaigns.value.items as IPrerollCampaign[])
...
const columns = ref<typeof defaultColumns>(filters?.[CampaignType.PREROLL]?.[AdEntityType.CAMPAIGNS] ? filters?.[CampaignType.PREROLL]?.[AdEntityType.CAMPAIGNS] : defaultColumns)
