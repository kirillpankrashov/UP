...
import TwitchExtension from '../TwitchExtension.vue'

describe('Streamer Settings TwitchExtension', () => {
	const factory = (props: any) => {
		const wrapper = mount(TwitchExtension, {
			global: {
	...
	it('doesn\'t render component if widget platform isn\'t Twitch', async () => {
		...
		expect(wrapper.find('[ddata-test="settings-twitch-extension-section"]').exists()).toBe(false)
	})

	it('calls the checkExtension method when the check button is clicked', async () => {
		const { wrapper, settingsStore } = factory({})
		...
		expect(settingsStore.checkExtension).toHaveBeenCalled()
	})
})
