...
describe('Streamer Campaigns AdsetInfo Info BrandAwareness CPA', () => {
	const factory = (props = {}) => {
	...
	describe('Payment type section', () => {
		it('renders payment type', () => {
			const { wrapper } = factory({
				payoutType: 'CPA',
			})

			expect(wrapper.find('[data-test="campaigns-info-payment-type"]').text()).toBe('CPA')
		})
	})
	...
	describe('Price per action section', () => {
		...
		it('does not render price per action when daily action limit is disabled', () => {
			const { wrapper } = factory({
				dailyActionLimit: {
					enabled: false,
					actionPrice: 100,
	...
	describe('Format section', () => {
		...
		it('shows fullscreen for Yandex FS format on YouTube', () => {
			const { wrapper, settingsStore } = factory({
				format: {
					id: AdFormat.YANDEX_FS,
					title: 'Some Format',
		...
		it('shows format title for non-Yandex formats', () => {
			const { wrapper } = factory({
				format: {
					id: 123,
					title: 'Custom Format',
	...
	describe('Advertiser category section', () => {
		it('renders advertiser category when available', () => {
			const { wrapper } = factory({
				campaign: {
					category: 'Gaming',
				},
		...
		it('renders fallback when category is not available', () => {
			const { wrapper } = factory({
				campaign: {
					category: null,
				},
	...
	describe('End date section', () => {
		it('calculates and displays days left correctly', () => {
		...
		it('shows 0 days when end date is in the past', () => {
	...
	describe('Currency formatting', () => {
		it('formats price with decimals for USD', async () => {
			...
			expect(wrapper.find('[data-test="campaigns-info-price-per-action"]').text()).toBe('100.5 USD')
		})
		...
		it('formats price without decimals for other currencies', async () => {
			...
			expect(wrapper.find('[data-test="campaigns-info-price-per-action"]').text()).toBe('100 USD')
		})
	})
