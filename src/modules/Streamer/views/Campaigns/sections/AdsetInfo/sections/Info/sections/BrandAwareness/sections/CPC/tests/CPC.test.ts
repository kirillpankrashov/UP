...
describe('Streamer Campaigns AdsetInfo Info BrandAwareness CPC', () => {
	const factory = (props = {}) => {
	...
	describe('Payment type section', () => {
		it('renders payment type', async () => {
			...
			expect(wrapper.find('[data-test="campaigns-info-payment-type"]').text()).toBe('CPC')
		})
	})

	describe('Price per click section', () => {
		it('renders price per click when daily action limit is enabled', async () => {
			const { wrapper } = factory({
				dailyActionLimit: {
					enabled: true,
					actionPrice: 100,
			...
			expect(wrapper.find('[data-test="campaigns-info-price-per-click"]').text()).toBe('100.0 USD')
		})

		it('does not render price per click when daily action limit is disabled', async () => {
			const { wrapper } = factory({
				dailyActionLimit: {
					enabled: false,
					actionPrice: 100,
			...
			expect(wrapper.find('[data-test="campaigns-info-price-per-click"]').exists()).toBe(false)
		})
	})
	...
	describe('Currency formatting', () => {
		it('formats price with decimals for USD', async () => {
			...
			expect(wrapper.find('[data-test="campaigns-info-price-per-click"]').text()).toBe('100.5 USD')
		})
		...
		it('formats price without decimals for other currencies', async () => {
			...
			expect(wrapper.find('[data-test="campaigns-info-price-per-click"]').text()).toBe('100 USD')
		})
	})
