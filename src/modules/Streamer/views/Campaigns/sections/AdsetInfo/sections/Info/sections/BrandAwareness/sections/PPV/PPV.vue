<template>
  <div
    data-test="campaigns-info-brand-awareness-ppv"
  ...
  >
    ...
    <div>
      ...
      <div
        data-test="campaigns-info-price-per-views"
        class="_text-m-regular"
      >
        {{ pricePerViewsText }}
      </div>
    </div>
    <!-- End of Price per views  -->
    ...
    <div>
      <div class="_text-s-regular mb-1 text-gray">
        {{ t('campaignSidebar.frequency') }}
      </div>
      <div
        data-test="campaigns-info-frequency"
        class="_text-m-regular"
      >
        {{ adset.frequency }}
      </div>
...
const pricePerViewsText = computed(() => {
	if(isExternalFormat(props.adset.format.id)) {
		if (props.adset.medianCpm) {
			return t('phrases.from') + ' ' + formatPayout(props.adset.creatorPayout.price) + ' ' + t('phrases.to') + ' ' + formatPayout(props.adset.medianCpm)
		}
		return t('phrases.from') + ' ' + formatPayout(props.adset.creatorPayout.price)
	}
	return formatPayout(props.adset.creatorPayout.price)
})
