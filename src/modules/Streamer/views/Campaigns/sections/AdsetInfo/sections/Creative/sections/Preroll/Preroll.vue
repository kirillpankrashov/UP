<template>
  <div data-name="campaigns-info-creative-preroll">
    ...
    <div class="_text-s-regular">
      {{ t('campaignSidebar.messageInDescription') }}
    </div>
    <div class="_text-m-regular">
      {{ adset.videoDescriptionText }}
    </div>
    <div class="mt-4 inline-flex flex-wrap gap-8">
      <TextLink
        class="no-underline"
        :href="adset.attachments.unit?.path"
        :download="adset.attachments?.unit?.basename"
        target="_blank"
      >
        {{ t('campaignSidebar.downloadCreative') }}
      </TextLink>

      <CopyLink
        :link="adset.videoDescriptionText"
        :label="t('campaignSidebar.copyDescription')"
      />
...
import { CopyLink, TextLink } from '@/components'
...
const copyLink = async () => {
	const link = await campaignStore.fetchPrerollLink(props.adset.slug)
