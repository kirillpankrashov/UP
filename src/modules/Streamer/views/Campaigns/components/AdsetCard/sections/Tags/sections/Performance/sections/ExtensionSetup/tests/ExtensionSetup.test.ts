...
vi.mock('@/core/hooks', () => ({
	useLocale: () => ({
		t: (key: string) => {
			if (key === 'links.twitchExtensionBrandis') return 'https://twitch.extension.link'
			return key
		},
	}),
}))

describe('Streamer Campaigns AdsetCard Tags Performance ExtensionSetup', () => {
	...
	it('does not render when brandis extension is enabled', async () => {
		...
		expect(wrapper.find('[data-name="campaigns-adset-card-tags-performance-extension-setup"]').exists()).toBe(false)
	})
	...
	it('renders when brandis extension is disabled', async () => {
		...
		expect(wrapper.find('[data-name="campaigns-adset-card-tags-performance-extension-setup"]').exists()).toBe(true)
