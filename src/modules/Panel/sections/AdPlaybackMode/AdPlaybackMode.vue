<template>
  <ElFormItem
    data-name="panel-ad-playback-mode"
    v-if="widget && appStore.domain?.name !== DomainName.STREAMO"
    :label="t('panel.settings.advertSettings.adPlaybackMode')"
  >
    <ElRadioGroup
    ...
    >
      <ElRadioButton label="auto">
        {{ t('panel.settings.advertSettings.playbackModeAuto') }}
      </ElRadioButton>

      <ElRadioButton label="manual">
        {{ t('panel.settings.advertSettings.playbackModeManual') }}
      </ElRadioButton>
    </ElRadioGroup>
  </ElFormItem>
</template>
